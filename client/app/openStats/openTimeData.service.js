...
angular.module('status2App').factory('OpenTimeData', function ($q, $http, TimeUtils) {
      ...
      get: function () {
        ...
        $http.get('/api/openStatistics').then(
          function (resp) {
            var data = resp.data;
            if (!data || !data.years) {
              srv._dataPromis.reject(new Error('Invalid opentime data.'));
              return;
            }
          ...
          function (resp) {
            srv._dataPromis.reject(new Error('Error during opentime data request.'));
          });
